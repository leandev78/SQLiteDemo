...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $datos = getComprobantesHistoricos($numSuministro, IDEMPRESA);

    echo json_encode($datos);
    exit();
}

// trae las 6 cuentas del período y ciclo más reciente (pueden ser más de una)
function getComprobantesHistoricos($cuenta, $idempresa) {
	
	$sql = "exec [dbo].[sp_getComprobantesHistoricosByEmpresaCuenta] :idempresa, :cuenta";
	$cant = 0;
    try {
        $dbCon = dbConnect();
